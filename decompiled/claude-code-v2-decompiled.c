...

{
  ...
  __printf_chk((double)g_result,2,"Global result: %.2f\n");
  g_secret = g_secret ^ 0xb9f9b978;
...
void subtract_numbers(int param_1,int param_2)

{
  g_flag = ((byte)(param_1 - param_2 >> 0x1f) & 0xfe) + 0x50;
  return;
}



void power(float param_1,uint param_2)

{
  ...
  if (0 < (int)param_2) {
    uVar1 = 0;
    g_result = 1.0;
    if (((param_2 & 1) == 0) || (uVar1 = 1, g_result = param_1, param_2 != 1)) {
      do {
        uVar1 = uVar1 + 2;
        g_result = g_result * param_1 * param_1;
      } while (param_2 != uVar1);
    }
    return;
  }
...
void xor_transform(uint param_1,uint param_2)

{
  g_secret = g_secret ^ param_1 ^ param_2;
  return;
}
